...
import { TriggerFilterDetails } from "./common/triggerDetails/TriggerFilterDetails";
import { NodeOptionContext } from "./NodeOptionProvider";

export default function TriggerSplitNode({
  data,
  isConnectable,
  ...
  const filterDetail = useMemo(
    () =>
      data.value.__typename === "WorkflowStateTriggerSplitActivityValue" &&
      data.value?.triggerFilter?.map((triggerFilter, parentIndex) =>
